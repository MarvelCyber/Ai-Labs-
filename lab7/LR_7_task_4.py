...
distance = [[0, 645, 868, 125, 748, 366, 256, 316, 1057, 382, 360, 471,428, 593, 311, 844, 602, 232, 575, 734, 521, 120,343, 312, 396],
...
# Список міст
cities = ['Вінниця', 'Дніпро', 'Донецьк', 'Житомир', 'Запоріжжя','Івано-Франківськ', 'Київ', 'Кропивницький', 'Луганськ', 'Луцьк', 'Львів', 'Миколаїв', 'Одеса','Полтава', 'Рівне', 'Сімферополь', 'Суми', 'Тернопіль','Ужгород', 'Харків', 'Херсон', 'Хмельницький', 'Черкаси','Чернівці', 'Чернігів']
if __name__ == '__main__':
    # Пошук відповіді задачі
    cityMap = CityMap(distance, len(distance[0]))
    colony = Colony(len(distance[0]))
    result = colony.find_route(cityMap, 0)
    ...
    plt.plot([i + 1 for i in range(25)], result[1], ms=12, marker='*', mfc='r', mec='black', mew=2, color='black', ls="--")
    plt.grid()
    plt.show()
